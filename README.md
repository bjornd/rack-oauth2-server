...
Rack::OAuth2::Server supports the use of bearer tokens in the form of the "assertion" grant_type
...
In order to verify the signatures of the assertion, you will need to create assertion Issuers.
...
$ ./script/console
  Loading development environment (Rails 2.3.8)
> issuer = Rack::OAuth2::Server.register_issuer(:identifier => "http://www.someidp.com", :hmac_secret => "foo", :public_key => "-----BEGIN RSA PUBLIC KEY-----\\n....\\n...=\\n-----END RSA PUBLIC KEY-----\\n")
```

When you call `register_issuer` it either registers a new issuer with these specific values, 
or if an issuer already exists, updates its other properties.
